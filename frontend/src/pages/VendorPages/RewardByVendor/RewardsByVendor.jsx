...
import ConfirmationModal from "../ModalsVendor/DeleteModalVendor";
import AddVendorReward from "../ModalsVendor/AddRewardVendor";
import {
  createRewardByVendor,
  deleteRewardByVendor,
  getAllVendorRewards,
  viewRewardByVendor,
} from "../../../features/vendorRewards";

const RewardsByVendor = () => {
  const dispatch = useDispatch();
  const { showToast } = useToast();

  const { VendorRewards, limit, page, isLoading, totalPages, totalRewards } =
    useSelector((state) => state.vendorReward);

  const rewards = VendorRewards || [];
  ...
  const isAllSelected =
    selectedRewardIds.length === rewards.length && rewards.length > 0;
  ...
  useEffect(() => {
    dispatch(getAllVendorRewards({ limit, page }));
  }, [dispatch]);

  const handlePageChange = (newPage) => {
    if (newPage < 1 || newPage > totalPages) return;
    dispatch(getAllVendorRewards({ page: newPage, limit }));
  };

  const handleLimitChange = (newLimit) => {
    dispatch(getAllVendorRewards({ page: 1, limit: Number(newLimit) }));
  };
  ...
  const openModalForEdit = (reward) => {
    dispatch(viewRewardByVendor(reward._id));
    setCurrentReward({ _id: reward._id, mode: "edit" });
    setIsModalOpen(true);
  };

  const openModalForView = (reward) => {
    dispatch(viewRewardByVendor(reward._id)).then(() => {
  ...
  const handleSave = (data) => {
    ...
    formData.append("description", data.rewardDescription);

    if (data.rewardImage) {
      formData.append("rewardImage", data.rewardImage);
    }
    dispatch(createRewardByVendor(formData))
      .unwrap()
      .then(() => {
        showToast("Reward created successfully!", "success");
        dispatch(
          getAllVendorRewards({ page: currentPage, limit: currentLimit })
        );
        setIsModalOpen(false);
      })
      .catch((err) => {
    ...
    if (isBulkDelete) {
      Promise.all(
        selectedRewardIds.map((id) =>
          dispatch(deleteRewardByVendor(id)).unwrap()
        )
      )
        .then(() => {
          showToast(
            `Deleted ${selectedRewardIds.length} rewards successfully!`,
            "success"
          );
          setSelectedRewardIds([]);
          closeDeleteModal();
      ...
      dispatch(deleteRewardByVendor(rewardToDelete._id))
        .unwrap()
        .then(() => {
  ...
  return (
                ...
                <table className="min-w-full divide-y divide-gray-200 ">
                  <thead>
                    <tr className="bg-gray-50">
                      ...
                      >
                        <div className="flex items-center gap-1">Stock </div>
                      </th>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {rewards.map((reward) => (
                      ...
                      >
                        ...
                        <td className="sm:px-6 py-1 sm:py-4 whitespace-nowrap text-xs sm:text-sm sm:font-medium text-gray-900">
                          <div className="flex items-center gap-4">
                            ...
                            <div
                              ...
                              onClick={() =>
                                setPreviewImage(reward.rewardImage)
                              }
                            >
                              <img
                        ...
                        <td className="hidden md:table-cell sm:px-6 sm:py-4 whitespace-nowrap">
                          <span
                            className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold
      ${
        reward.stockStatus === "in_stock"
    ...
    `}
        ...
        ${reward.stockStatus === "in_stock" ? "bg-green-600" : "bg-red-600"}
      `}
                            />
                            {reward.stockStatus === "in_stock"
                              ? "In Stock"
            ...
            <div className="flex items-center justify-center min-h-[600px]">
              <div className="p-16 text-center text-gray-500">
                ...
                <p className="mt-1 text-sm text-gray-500">
                  Get started by adding a new reward using the "Add New Reward"
                  button.
                </p>
              </div>
            </div>
...
export default RewardsByVendor;
