...
import AdminRewardModal from '../../Admin/AdminModals/RewardsAdminModal';
import { createRewardByAdmin, getAllRewardsByAdmin } from '../../../features/adminRewardSlice';
import { getAllVendorRewards } from '../../../features/vendorRewards';
...
const RewardsByVendor = () => {
    ...
    const {VendorRewards, limit, page, isLoading, totalPages, totalRewards } = useSelector((state)=> state.vendorReward);
   ...
   const handleLimitChange = (newLimit) => {
    ...
    const handleSave = (data) => {
        ...
        dispatch(createRewardByAdmin(formData))
            .unwrap()
            .then(() => {
        ...
        if (isBulkDelete) {
            Promise.all(selectedRewardIds.map(id => dispatch(deleteReward(id)).unwrap()))
                .then(() => {
                    ...
                    setSelectedRewardIds([]);
                    dispatch(getAllVendorRewards({ page: currentPage, limit: currentLimit }));
                    closeDeleteModal();
                })
                .catch((err) => {
                ...
                <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
                    {isLoading ? (
                        ...
                        <td className="hidden md:table-cell sm:px-6 sm:py-4 whitespace-nowrap text-sm text-gray-600">
                         ...
                         <td className="hidden lg:table-cell sm:px-6 sm:py-4 whitespace-nowrap">
  <select
    value={reward.stockStatus}
    onChange={(e) =>
      handleStockStatusChange(reward._id, e.target.value)
    }
    ...
    `}
  >
    <option value="in_stock" className="text-green-700">
      In Stock
    </option>
    <option value="out_of_stock" className="text-red-700">
      Out of Stock
    </option>
  </select>
</td>
