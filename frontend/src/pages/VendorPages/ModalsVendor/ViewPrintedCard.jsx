  ...
  const Row = ({ label, value, badge = false }) => {
    ...
    return (
      <div className="grid grid-cols-[140px_1fr] gap-2 py-1.5">
        ...
        {badge ? (
          <span className="inline-block w-fit px-2 py-0.5 rounded-full
            text-[11px] font-semibold bg-indigo-100 text-indigo-700">
            {value}
          </span>
        ) : (
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-3">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-xl max-h-[90vh] flex flex-col">
        ...
        <div className="px-4 py-3 space-y-3 overflow-y-auto">
          ...
          {/* Reward */}
          {hasValue(card.reward?.name) && (
            <Section title="Reward">
              <Row label="Reward Name" value={card.reward.name} />
            </Section>
          )}
          ...
          {/* Recipient */}
          {(hasValue(card.recipient_name) || hasValue(card.recipient_email)) && (
            <Section title="Recipient">
              <Row
                label="Full Name"
                value={`${card.recipient_name}${card.recipient_last_name ? ` ${card.recipient_last_name}` : ""}`}
              />
              <Row label="Email" value={card.recipient_email} />
            </Section>
          )}
