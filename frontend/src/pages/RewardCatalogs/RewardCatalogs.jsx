...
import { claimReward, getStudentRewards } from '../../features/studentRewardsSlice';
import { Gift, Send, Sparkles } from "lucide-react";
import { useToast } from "../../context/ToastContext";

 
 
const RewardBadge = ({ name, rewardImage, totalPoints, completedPoints, percentage,  onClick
 }) => {
  ...
  transition-all duration-300 hover:scale-[1.04] hover:-translate-y-2 hover:shadow-xl hover:shadow-purple-200">
      <div className="absolute top-full mt-2 hidden group-hover:block bg-gray-800 text-white text-xs rounded-lg p-3 w-48 z-10 shadow-lg">
        <div className="space-y-1">
          ...
          <p><span className="font-semibold">Completed:</span> {completedPoints}</p>
          <p><span className="font-semibold">Remaining:</span> {totalPoints - completedPoints}</p>
          <p><span className="font-semibold">Progress:</span> {percentage}%</p>
        </div>
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-white p-6 rounded-xl w-80 shadow-lg">
        <h2 className="text-lg font-semibold text-[#6955A5]">
          Claim Reward
        </h2>
        <p className="text-sm text-gray-600 mt-2">
          Are you sure you want to claim <span className="font-medium">{rewardName}</span>?
        </p>
