...
import VendorAdminModal from "../AdminModals/VendorAdminModel";
import { createVendorByAdmin, deleteVendorByAdmin, getAllVendorsByAdmin, getSingleVendorByAdmin, updateVendorByAdmin } from "../../../features/adminVendorSlice";
...
const AdminManageVendors = () => {
  ...
  const {
    ...
    totalVendors,
    singleVendor,
  } = useSelector((state) => state.adminVendors);
  ...
  useEffect(() => {
    dispatch(getAllVendorsByAdmin({ limit, page }));
  }, [dispatch]);
  ...
  const handleLimitChange = (newLimit) => {
    dispatch(getAllVendorsByAdmin({ page: 1, limit: Number(newLimit) }));
  };

  const handlePageChange = (newPage) => {
    if (newPage < 1 || newPage > totalPages) return;
    dispatch(getAllVendorsByAdmin({ page: newPage, limit }));
  };
                            ...
                            onClick={async () => {
                              try {
                                const res = await dispatch(
                                  getSingleVendorByAdmin(vendor._id)
                                ).unwrap();
