  ...
  try {
    ...
    switch (role) {
      ...
      case "admin":
        ...
        if (data.university) user.university = data.university;
        if (data.colleges) {
          const incomingColleges = Array.isArray(data.colleges)
            ? data.colleges
            : data.colleges.split(",").map(id => id.trim());

          const existingCollegeIds = (user.colleges || []).map(id => id.toString());

          const uniqueNewColleges = incomingColleges.filter(
            id => !existingCollegeIds.includes(id)
          );
    ...
    await user.save();
    const populatedUser = await User.findById(user._id).select("-password").populate("university", "name").populate("college", "name").populate("colleges", "name");
    ...
    if (role === "admin") {
      responseUser = {
        ...
        university: populatedUser.university?._id || null,
        university_name: populatedUser.university?.name || null,
        colleges: populatedUser.colleges?.map(c => c._id) || [],
        colleges_names: populatedUser.colleges?.map(c => c.name) || [],
      };
    }
